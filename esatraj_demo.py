...
class Ui_ESATrajWindow(object):
 ...
 #----------- initiazation ----------
    ...
    def filter_trajectory(self, traj):
        ...
        return filtered_datax, filtered_datay, filtered_dataz


    def update_3d_plot(self):
        if FILTER_PLOT:
            plotx, ploty, plotz = self.filter_trajectory(self.trajectory3d)
        else:
            plotx = np.array(self.trajectory3d[:,0])
            ploty = np.array(self.trajectory3d[:,1])
            plotz = np.array(self.trajectory3d[:,2])

        self.plot3d_canvas.ax.plot(plotx, ploty, plotz, color = 'b', linewidth=1.0)
        min_x = np.amin(plotx)
        min_y = np.amin(ploty)
        min_z = np.amin(plotz)
        max_x = np.amax(plotx)
        ...
        if self.com_port_name == 'Dummy':
            ...
            def receive_traj():
                while self.connected:
                    ...
                    if data[0] == b's':
                        print('start') 
                        self.cur_p = np.array([0,0,0], dtype=float)
                        self.cur_q = quaternion.from_float_array([1,0,0,0])
                        self.receiving = True
                    elif data[0] == b'e':
                        print('end')
                        self.receiving = False 
                    ...
                    if data[0] == b'y' and len(data) >= 8:
                        ...
                        temp_dis = np.matmul(quaternion.as_rotation_matrix(self.cur_q), delta_p.T).T
                        self.cur_p = self.cur_p + temp_dis
                        self.cur_q = self.cur_q * quaternion.from_float_array(delta_q).normalized()

                    self.trajectory3d = np.append(self.trajectory3d,[[self.cur_p[0],self.cur_p[1],self.cur_p[2]]],axis=0)
